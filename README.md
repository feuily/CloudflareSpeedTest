...
默认会把**前 `-p` 条结果**保存到当前目录下的 `result.csv` 文件中，用**记事本/表格软件**打开，格式如下：
...
> [!NOTE]
> _如果结果文件因路径无效、权限不足等原因写入失败，软件会输出警告，但仍会继续在命令行显示测速结果。_
...
> _大家可以按自己需求，对结果**进一步筛选处理**，或者去看一看进阶使用**指定过滤条件**！_
...
参数：
    -n 200
        ...
        在 HTTPing 模式下，如果并发设置过高，程序会自动降到更保守的上限并稍微平滑启动节奏，以降低被服务商限速/识别为扫描的概率。
    -t 4
        延迟测速次数；单个 IP 延迟测速的次数；(默认 4 次)
    -dn 10
        下载测速数量；延迟测速并排序后，从最低延迟起下载测速的数量；(默认 10 个)
        当候选 IP 数量较多时，程序会先做短时预检，再对预检排名靠前的 IP 做完整确认，以更快筛出高质量 IP。
    -dt 10
    ...
    -qps 0
        全局请求速率限制；0 表示 HTTPing 自动限流，正数表示固定 QPS，负数表示关闭限流；(默认 0)
        自动模式会在持续失败时逐步降速，连续成功后再缓慢恢复。
        下载测速阶段会实时显示当前正在测试的 IP 和即时速度。

    -p 10
        显示结果数量；测速后直接显示指定数量的结果，为 0 时不显示结果直接退出；(默认 10 个)
        同时也会控制写入结果文件的条数；为 0 时仅关闭命令行输出，不截断结果文件
    -f ip.txt
    ...
    -o result.csv
        ...
        如果结果文件创建或写入失败，软件会输出警告，但不会影响命令行结果输出
...
> 当前版本对 IPv6 网段会按**每个网段随机抽取一个落在该 CIDR 内的地址**进行测速，以避免宽前缀网段导致地址数量失控。
